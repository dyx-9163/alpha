...
3. 每次结束前，把本轮“问题”和“结论”精简追加到 `memory.md`。**追加前必须先执行下方"memory.md 写入守卫"流程**，确保主文件只保留当天条目且不超过阈值。只记录可复用事实、决策和待办，不写密码、token、私钥、完整连接串、长日志。
4. 如果 `memory.md` 不存在，按下方"memory.md 写入守卫"中的初始化规则创建；历史归档目录为 `memory/`。
5. 修改 `web/src` 下页面、布局、组件或样式前，必须先读取 `design/ant-design-system-portable202606.md`。
6. 每次代码更新完成且必要验证通过后，自动执行 `git add`、`git commit` 并推送到当前 GitHub 远端分支；如存在未授权改动、暂停指令、分支/凭据/网络阻塞或推送风险，必须先向用户说明并确认。
...
`memory.md` 是**当天滚动窗口**，不是无限追加的历史日志。每次向 `memory.md` 写入（含首次创建、追加、编辑）前，必须按以下顺序执行守卫流程；跳过即视为违规。
...
### 守卫步骤
1. **读取当前 `memory.md`**（若不存在则跳到步骤 4 初始化）。
2. **识别日期段**：扫描所有 `## YYYY-MM-DD` 二级标题，区分"当天"与"非当天"。
3. **归档判定**：满足以下任一条件即执行归档：
   - 存在非当天的 `## YYYY-MM-DD` 段；
   - 主文件行数 > 200 或大小 > 50 KB。
4. **归档执行**（仅在需要时）：
   - 确保 `memory/` 目录存在；
   - 将每个非当天日期段整体移入 `memory/YYYY-MM-DD.md`（同名覆盖）；
   - 若当天段因体积超限仍需裁剪，保留最新条目直到 ≤ 阈值，旧条目并入对应日期归档；
   - 归档完成后重写 `memory.md`，仅保留头部说明 + 当天段。
5. **写入新条目**：在当天 `## YYYY-MM-DD` 段下追加 `- 问题：... / - 结论：...`；若当天段不存在则新建。
6. **写入后复核**：再次检查行数与大小，仍超阈值则回到步骤 3。
...
### 归档文件格式（memory/YYYY-MM-DD.md）
```markdown
# AIFAR Memory Archive — YYYY-MM-DD
...
### 注意事项
- 读取历史记忆时，先读 `memory.md`（当天），再按需 `glob memory/*.md` 找特定日期归档；不要一次性加载全部归档。
- 归档文件名严格为 `YYYY-MM-DD.md`，便于排序与检索。
- 守卫流程是 agent 约定，不依赖外部脚本；任何能写文件的 agent 都必须遵守。
