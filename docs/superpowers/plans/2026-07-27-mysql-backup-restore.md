...
- `local_infile` must be restored in a finally path. An unreachable target records a non-secret reconciliation marker, fails the task with `MYSQL_LOCAL_INFILE_RESTORE_FAILED`, and blocks later MySQL lifecycle actions until reconciliation succeeds.
...
- [ ] Add an unreachable-finally test proving the task returns `MYSQL_LOCAL_INFILE_RESTORE_FAILED`, writes a non-secret reconciliation marker to instance metadata, and does not report restore success.
