...
- [ ] Add focused tests for a normal job error, cancellation while waiting for a slot, cancel/panic precedence, exact terminal event counts, lock/slot release, and sensitive panic masking.
- [ ] Run `go test -count=1 ./internal/worker` from `backend/`; record which new assertions fail and why.
- [ ] Make the minimal terminal-claim, cleanup, or masking changes needed for the tests; do not add leases or retry semantics.
- [ ] Run `go test -count=10 ./internal/worker` and `go test -count=1 ./internal/i18n`.
- [ ] Commit only this scope as `fix(worker): make terminal outcomes atomic`.
...
- [ ] Add failing tests for wildcard/empty/non-loopback addresses, loopback IPv4/IPv6/localhost, explicit false, and equal current/previous secrets under override.
- [ ] Run `go test -count=1 ./internal/config` and the focused Node test; confirm the new cases fail for policy reasons.
- [ ] Implement one shared loopback decision and make both backend validation and dev launchers honor it.
- [ ] Keep distributable password/JWT/credential values blank and document the exact production behavior.
- [ ] Run config and script tests, then commit as `fix(security): fail closed outside loopback`.
...
- [ ] Add failing tests for wrong current/no previous, wrong previous rollback, mixed current/previous ciphertext, read-only no-write behavior, idempotent second start, and secret-free errors.
- [ ] Run `go test -count=1 ./internal/store ./internal/config`; confirm the new validation cases fail.
- [ ] Reuse one target list for validation and rotation, close query rows before updates, and keep all rotation writes in one transaction.
- [ ] Wire validation/rotation before bootstrap and HTTP startup; clear the previous derived key after success.
- [ ] Run `go test -count=1 ./internal/store ./internal/config ./cmd/aifar-server`, then commit as `fix(store): validate and rotate credential secrets`.
...
- [ ] Add tests for empty success retention, unchanged revision, equal-version later collection, real SSE envelope parsing, and concurrent old GET versus newer SSE.
- [ ] Run the focused Vitest file and capture expected failures for any missing behavior.
- [ ] Implement the smallest merge/freshness changes; do not add tombstones, TTL, polling, or backend cleanup.
- [ ] Run `pnpm test:web`, then commit as `fix(web-realtime): harden status snapshot cache`.
...
- [ ] Add pure behavior tests for cache keys, image identity/deduplication, Docker realtime merge, bootstrap degradation, Docker requests, all Runtime requests, artifact/config/format/selectors, and virtual log viewport calculations.
- [ ] Run the focused tests; existing untested helpers may pass characterization tests, but any new correction must be proven RED before implementation.
- [ ] Correct only behavior exposed by failing tests and finish type-safe wiring; do not add a component-test stack or further controller extraction.
- [ ] Run `pnpm test:web` and `pnpm web:build`.
- [ ] Commit the complete compiling extraction as `refactor(web): finish containers runtime extraction`.
...
- [ ] Add fixture tests for malformed, duplicate, quoted, explicit-empty, loopback, system-toolchain, and missing-toolchain cases.
- [ ] Run the focused tests and confirm parser/toolchain gaps fail.
- [ ] Implement equivalent shell and PowerShell semantics without sourcing or evaluating arbitrary config content.
- [ ] Verify shell/PowerShell syntax and `git ls-files --eol` output.
- [ ] Commit as `fix(tooling): make startup and toolchain behavior portable`.
...
- [ ] Add thin-fixture tests for unsafe defaults before copy, archive command failure, staging cleanup, missing platform, missing archive, stale-version false positive, extraction, and checksum mismatch.
- [ ] Run `pnpm test:scripts` and confirm new cases fail.
- [ ] Move security validation before resource copy, wrap staging in unconditional cleanup, make archive failures fatal, and verify only the current package version.
- [ ] Remove the duplicate release call after `package` in `test:local`.
- [ ] Run `pnpm test:scripts` and a thin fixture package/verify cycle, then commit as `fix(release): require verifiable platform archives`.
...
- [ ] Add CI steps for backend/web/script tests, web/backend builds, Ubuntu Worker/Store race tests, and Windows/Linux startup syntax/fixture checks.
- [ ] Synchronize project guidance and task checkboxes with the current source; append the final reusable conclusion to `memory.md` without secrets or long logs.
- [ ] Run `pnpm test`, `pnpm test:web`, `pnpm test:scripts`, `pnpm web:build`, `pnpm backend:build`, and `git diff --check`.
- [ ] Run one final `pnpm test:local`; confirm both current-version archives exist, extract, and verify.
- [ ] Commit as `ci/docs: enforce scheme-0 gates and sync guidance`.
- [ ] Dispatch a whole-branch code review, fix all Critical/Important findings in one fix wave, and re-run affected tests plus the final fast gate.
