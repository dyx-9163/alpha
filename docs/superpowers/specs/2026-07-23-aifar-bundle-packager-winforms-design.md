...
用户通过对应的选择按钮手动指定路径；路径框仅展示选择结果，不允许直接输入。输出 ZIP 始终必选；选择 Java 服务时要求 Java 源码根目录，选择 `web-vue3` 时要求 Web `dist`，未选择某类服务时对应来源路径保留但不参与校验。取消选择对话框不会修改原有选择结果。

程序不创建 `%LocalAppData%\AIFAR\BundlePackager\settings.json` 或其他路径配置文件。服务选择不持久化，每次启动默认全选。

## 打包数据流

1. 确认输出 ZIP 和至少一个服务已选择，仅对所选服务类别要求并标准化 Java/Web 来源路径。
...
- 失败不覆盖已有输出，成功才替换。
- staging 与临时 ZIP 清理。
- 三个路径启动时均为空；输出 ZIP 始终必选，Java/Web 来源路径只在选择对应服务类别时必选。
