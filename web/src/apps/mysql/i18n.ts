...
export const mysqlMessages = {
  zh: {
    ...
    hint: '单体使用单台服务器；InnoDB Cluster 需要多台服务器。安装器会先安装基础 MySQL 和离线包内置 MySQL Shell，再在第一台服务器执行集群初始化。',
    version: '版本',
  ...
  en: {
    ...
    hint: 'Standalone uses one server; InnoDB Cluster requires multiple servers. The installer installs base MySQL and the bundled MySQL Shell first, then bootstraps the cluster from the first server.',
    version: 'Version',
