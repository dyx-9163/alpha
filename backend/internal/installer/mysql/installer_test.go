...
func TestInstallerUploadsBundleAndRunsMySQLRouterScript(t *testing.T) {
	...
	remote := &fakeRemote{}
	installer := NewInstaller(remote)
	err := installer.InstallRouter(context.Background(), store.Server{Name: "router-1", Host: "10.0.0.9", DeployDir: "/aifar/apps"}, Bundle{
		Version:     "8.0.36",
		ArchivePath: archive,
		RPMPaths:    []string{rpm},
...
func TestMySQLRouterScriptsRenderTemplates(t *testing.T) {
	install, err := installRouterScript(RouterInstallScriptRequest{
		...
		ArchivePath:       "/tmp/mysql.tar",
		InstallRoot:       "/aifar/apps/mysql-router/8.0.36",
		BasePort:          6446,
		BootstrapHost:     "10.0.0.1",
		BootstrapPort:     3306,
		BootstrapUser:     "root",
	...
	uninstall, err := uninstallRouterScript("8.0.36", "/aifar/apps/mysql-router/8.0.36", 6446)
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(uninstall, "{{") {
	...
	if !strings.Contains(uninstall, `SERVICE_NAME="aifar-mysql-router-$BASE_PORT"`) || !strings.Contains(uninstall, "BASE_PORT=6446") {
