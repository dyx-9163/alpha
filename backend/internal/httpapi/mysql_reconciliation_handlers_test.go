...
func TestMySQLReconciliationHandlerOwnerOnlyCreatesRawLockedAuditedTask(t *testing.T) {
	...
	case call := <-module.calls:
		if call.instance.ID != instance.ID || call.taskID != taskID {
...
func (m *reconciliationHandlerModule) Reconcile(ctx context.Context, instance store.AppInstance, _ string, taskID string, _ mysqlapp.Logger) error {
	m.calls <- reconciliationHandlerCall{instance: instance, taskID: taskID}
	select {
	case <-m.release:
		return nil
	case <-ctx.Done():
