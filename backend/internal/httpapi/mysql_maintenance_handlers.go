...
func (a *API) clearMySQLMaintenance(w http.ResponseWriter, r *http.Request) {
	...
	stepTitle := i18n.Text(lang, "mysql.maintenance.clearStep")
	plan := []registry.InstallStepPlan{{Target: target, Name: "clear-maintenance", Title: stepTitle, Order: 1}}
	...
	task, err = a.tasks.StartExistingWithLanguage(task, lang, func(ctx context.Context, log worker.Logger) error {
		...
		log.StartStep(target, "clear-maintenance", stepTitle, 1)
