...
type Item struct {
	...
	ResourceVersionPattern string              `json:"resourceVersionPattern,omitempty"`
...
func DefinitionFromManifest(manifest registry.Manifest) Definition {
	return Definition{
		...
		ResourceVersionPattern: manifest.ResourceVersionPattern,
...
func BuildWithModules(resources []store.Resource, modules []registry.Module, lang string) map[string]Item {
	...
	for _, module := range modules {
		...
		item := Item{
			...
			ResourceVersionPattern: def.ResourceVersionPattern,
		...
		for _, res := range resources {
			...
			if !versionAllowed(def, res.Version) {
				continue
			}
...
func ResolveResources(def Definition, resources []store.Resource, version string) (string, []store.Resource) {
	...
	for _, res := range resources {
		if res.App == app && versionAllowed(def, res.Version) && res.Version == selected {
			out = append(out, res)
		}
	}
	return selected, out
...
func versionAllowed(def Definition, version string) bool {
	pattern := strings.TrimSpace(def.ResourceVersionPattern)
	if pattern == "" {
		return true
	}
	ok, err := regexp.MatchString(pattern, version)
	if err != nil {
