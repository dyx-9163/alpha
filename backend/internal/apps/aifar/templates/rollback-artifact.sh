...
SNAPSHOT_DIR="$ROLLBACK_DIR/services/$SERVICE_NAME/snapshot"
TMP_DIR="$INSTALL_ROOT/.rollback-$ROLLBACK_ID-$SERVICE_NAME-$$"
...
replicas_for_service() {
  service="$1"
  value="$(desired_replicas_for_service "$service")"
  case "$value" in ""|*[!0-9]*) value=1 ;; esac
  printf "%s" "$value"
}
...
resource_value() {
  ...
  key="$2"
  fallback="$3"
  value="$(read_env_value "$ENV_DIR/resource.$service.env" "$key" "")"
  [ -n "$value" ] || value="$(read_env_value "$ENV_DIR/$service.env" "$key" "")"
...
snapshot_current_state() {
  mkdir -p "$SNAPSHOT_DIR" "$ROLLBACK_DIR/snapshot"
  service_env="$ENV_DIR/$SERVICE_NAME.env"
  [ -f "$service_env" ] && cp "$service_env" "$SNAPSHOT_DIR/before.env" || true
  [ -f "$INSTALL_ROOT/runtime/agent/runtime-spec.json" ] && cp "$INSTALL_ROOT/runtime/agent/runtime-spec.json" "$ROLLBACK_DIR/snapshot/before-runtime-spec.json" || true
}

restore_previous_runtime() {
  service_env="$ENV_DIR/$SERVICE_NAME.env"
  [ -f "$SNAPSHOT_DIR/before.env" ] && cp "$SNAPSHOT_DIR/before.env" "$service_env" || true
  [ -f "$ROLLBACK_DIR/snapshot/before-runtime-spec.json" ] && cp "$ROLLBACK_DIR/snapshot/before-runtime-spec.json" "$INSTALL_ROOT/runtime/agent/runtime-spec.json" || true
...
apply_artifact() {
  ...
  [ -f "$ARTIFACT_REMOTE" ] || fail "target release artifact is missing: $ARTIFACT_REMOTE"
  actual="$(sha256sum "$ARTIFACT_REMOTE" | awk '{print $1}')"
  [ "$actual" = "$ARTIFACT_SHA256" ] || fail "target release artifact checksum mismatch for $ARTIFACT_FILE"
...
write_desired_replicas_env() {
  ...
  for service in $SERVICE_ORDER; do
    ...
    [ -f "$file" ] || continue
    set_env AIFAR_DESIRED_REPLICAS "$value" "$file"
  done
}
...
JSON
  ...
  for service in $SERVICE_ORDER; do
    ...
    mkdir -p "$log_dir"
    if [ "$first_deployment" = "1" ]; then first_deployment=0; else printf ",\n" >> "$spec"; fi
...
JSON
  ...
  for service in $SERVICE_ORDER; do
    ...
    if [ "$first_service" = "1" ]; then first_service=0; else printf ",\n" >> "$spec"; fi
...
apply_artifact
ensure_service_runtime_env
image="$(read_env_value "$ENV_DIR/$SERVICE_NAME.env" APP_IMAGE "")"
[ -n "$image" ] || fail "service image is empty: $SERVICE_NAME"
docker build -t "$image" "$APP_DIR/$SERVICE_NAME"
if ! reconcile_runtime; then
  restore_previous_runtime
  fail "AIFAR runtime rollback reconcile failed: $SERVICE_NAME"
fi
write_model_manifest
rm -rf "$TMP_DIR"
echo "AIFAR Deployment rollback completed: $SERVICE_NAME -> $REVISION"
