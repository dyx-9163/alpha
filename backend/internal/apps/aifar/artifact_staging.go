...
func prepareInstallArtifactStage(ctx context.Context, remote installerkit.Remote, server store.Server, installRoot string, stage installArtifactStage, log Logger) error {
	command := strings.Join([]string{
		...
		`[ -d "/" ] && [ ! -L "/" ] || { echo "AIFAR install staging filesystem root is unsafe" >&2; exit 64; }`,
		`root_real="$(readlink -f "/")"`,
		`[ "$root_real" = "/" ] || { echo "AIFAR install staging filesystem root changed" >&2; exit 64; }`,
		...
		`  esac`,
		`  [ -n "$component" ] && [ "$component" != "." ] && [ "$component" != ".." ] || { echo "AIFAR install staging root component is unsafe" >&2; exit 64; }`,
		`  next="$current/$component"`,
		`  if [ -e "$next" ] || [ -L "$next" ]; then`,
		`    [ -d "$next" ] && [ ! -L "$next" ] || { echo "AIFAR install staging root component is unsafe" >&2; exit 64; }`,
		`  else`,
		`    mkdir -- "$next"`,
		`  fi`,
		`  [ -d "$next" ] && [ ! -L "$next" ] || { echo "AIFAR install staging root component is unsafe" >&2; exit 64; }`,
		`  next_real="$(readlink -f "$next")"`,
		`  [ "$next_real" = "$next" ] || { echo "AIFAR install staging root component escaped" >&2; exit 64; }`,
		`  current="$next"`,
		`done`,
		`[ "$current" = "$install_root" ] || { echo "AIFAR install staging root changed" >&2; exit 64; }`,
