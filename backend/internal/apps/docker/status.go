...
func (i Inspector) Check(ctx context.Context, server store.Server, version string, log Logger) (StatusResult, error) {
	...
	legacyInstallRoot := installerkit.LegacyInstallRoot(deployDir, "docker", version)
	result, err := i.remote.Run(ctx, server, dockerStatusCommand(installRoot, legacyInstallRoot))
...
func dockerStatusCommand(installRoot, legacyInstallRoot string) string {
	return "sh -s <<'AIFAR_DOCKER_STATUS'\n" + `#!/usr/bin/env sh
set -u

INSTALL_ROOT=` + installerkit.ShellQuote(installRoot) + `
LEGACY_INSTALL_ROOT=` + installerkit.ShellQuote(legacyInstallRoot) + `
STATUS="missing"
DOCKER_VERSION=""
COMPOSE_VERSION=""
UNIT_EXISTS="false"
