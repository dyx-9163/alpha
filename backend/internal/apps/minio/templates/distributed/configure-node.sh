...
if ! ($SUDO systemctl is-active --quiet "$SERVICE_NAME" || $SUDO systemctl is-active --quiet "$LEGACY_SERVICE_NAME"); then
  ...
  $SUDO journalctl -u "$LEGACY_SERVICE_NAME" -n 120 --no-pager || true
  exit 1
fi
