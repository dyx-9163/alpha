...
func TestInstallerUploadsBundleAndRunsMySQLScript(t *testing.T) {
	...
	err := installer.Install(context.Background(), store.Server{Name: "db-1", Host: "10.0.0.4", DeployDir: "/aifar/apps"}, Bundle{
		...
		ArchivePath: archive,
		RPMPaths:    []string{rpm},
	}, InstallOptions{Port: 3307, RootUser: "root", RootPassword: "Oversea.123"}, installerTestLogger{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMySQLStandaloneScriptsRenderTemplates(t *testing.T) {
	...
	if !strings.Contains(uninstall, `SERVICE_NAME="aifar-mysql"`) || !strings.Contains(uninstall, `LEGACY_SERVICE_NAME="aifar-mysql-$PORT"`) || !strings.Contains(uninstall, "PORT=3307") {
