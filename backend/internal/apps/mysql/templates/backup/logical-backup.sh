...
exec python3 - "{{.WorkDir}}" "{{.DumpDir}}" "{{.MySQLShell}}" "{{.Threads}}" "{{.MaxRateMBps}}" <<'PY'
import os
import stat
import subprocess
import sys

work_dir, dump_dir, mysqlsh, threads, max_rate = sys.argv[1:]
...
''' % dump_fd
try:
    js_fd = os.open("logical-backup.js", os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW, 0o600, dir_fd=work_fd)
